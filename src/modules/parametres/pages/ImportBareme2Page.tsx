...
import {
  commitBareme2Import,
  downloadBareme2ImportTemplate,
  parseBareme2ExcelFile,
  type Bareme2ImportPreviewRow,
  type Bareme2ImportResult,
} from '../importBareme2'
import { BAREME2_FIELDS, headersLine, IMPORT_STRUCTURES } from '../importStructures'
import { useParametres } from '../ParametresContext'

export function ImportBareme2Page() {
  const { store, reload, setCollection } = useParametres()
  const inputRef = useRef<HTMLInputElement>(null)
  const def = IMPORT_STRUCTURES.bareme2
  const line = headersLine(def)
  const installedCount = store.baremes2.length
  const [copied, setCopied] = useState(false)
  const [fileName, setFileName] = useState('')
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  const [preview, setPreview] = useState<Bareme2ImportPreviewRow[]>([])
  const [result, setResult] = useState<Bareme2ImportResult | null>(null)
  ...
  async function onFileChange(file: File | null) {
    ...
    try {
      const rows = await parseBareme2ExcelFile(file)
      setPreview(rows)
    } catch (err) {
  ...
  function onImport() {
    ...
    setError('')
    try {
      const summary = commitBareme2Import(preview)
      reload()
      setResult(summary)
    } catch (err) {
  ...
  function supprimerBaremeInstalle() {
    ...
    const lockedCount = store.baremes2.filter((row) => row.valide).length
    if (lockedCount === installedCount) {
      window.alert(
        'Toutes les lignes du Barème 2 sont validées : suppression impossible.',
      )
      return
    }
    ...
    <div className="import-personnel-page import-structure-page">
      <header className="page-header">
        <div>
          ...
          <h1>Barème 2</h1>
          <p className="lede">
            Importez un fichier Excel Barème 2. Les lignes avec un grade déjà
            existant seront mises à jour dans Fichiers → Barème 2. Les lignes
      ...
      <section className="import-personnel-panel">
        <div className="import-structure-meta">
          ...
          <p>{BAREME2_FIELDS.length} colonnes Excel</p>
        </div>
        ...
        <div className="import-personnel-actions">
          ...
          <button
            type="button"
            className="btn-ghost"
            onClick={() => downloadBareme2ImportTemplate()}
          >
            <Download size={16} />
        ...
        <div className="import-personnel-table-wrap">
          <table className="data-table import-personnel-table">
            ...
            <tbody>
              {BAREME2_FIELDS.map((field, index) => (
                <tr key={field.key}>
                  <td>{index + 1}</td>
                  <td>
          ...
          <div className="import-personnel-result" role="status">
            ...
            {syncedCount > 0 && (
              <p>
                ...
                <Link to="/parametres/fichiers/bareme2">Fichiers → Barème 2</Link>
                .
              </p>
            )}
