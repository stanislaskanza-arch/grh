...
type Props = {
  ligne: PaieMensuelleB1Item | null
  onClose: () => void
}
...
function fullName(ligne: PaieMensuelleB1Item) {
  return [ligne.nom, ligne.postnom, ligne.prenom].filter(Boolean).join(' ').trim()
}

/** Contenu encodé dans le QR de la fiche de paie. */
export function buildPaieQrPayload(ligne: PaieMensuelleB1Item): string {
...
export function FichePaieB1Modal({ ligne, onClose }: Props) {
  const [qrDataUrl, setQrDataUrl] = useState('')

  const qrPayload = useMemo(
    () => (ligne ? buildPaieQrPayload(ligne) : ''),
    [ligne],
  )
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="fiche-paie-b1-title"
        ...
        >
          <header className="fiche-sheet-header">
            <div className="fiche-brand">
              ...
              <div>
                ...
                <small>Fiche de paie — Barème 1</small>
              </div>
            </div>
          ...
          <section className="fiche-hero-block fiche-hero-block-paie">
            <div className="fiche-hero-text">
              ...
              <h2 id="fiche-paie-b1-title">{name}</h2>
