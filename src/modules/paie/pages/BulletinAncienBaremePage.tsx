...
function findBareme1ForGrade(
  gradeCode: string,
  baremes1: Bareme1Item[],
): Bareme1Item | null {
  const code = normalizeToken(gradeCode)
  if (!code) return null
  return (
    baremes1.find((b) => {
      const g = normalizeToken(b.grade)
      const lg = normalizeToken(b.libelleGrade)
      return g === code || lg === code || g.startsWith(code) || code.startsWith(g)
    }) ?? null
  )
}
...
export function BulletinAncienBaremePage() {
  const [b1Rows, setB1Rows] = useState<PaieMensuelleB1Item[]>(
  ...
  useEffect(() => {
    ...
    function refresh() {
      ...
      refreshing = true
      try {
        setB1Rows(loadPaieStore().paieMensuelleB1)
  ...
  const bulletins = useMemo(() => {
    ...
    return bulletinsMois.map((row) => {
      ...
      const isActif = agent
        ? isPersonnelActif(agent, refs)
        : false
      const grade = gradeCodeOnly(row.grade)
      const bareme = findBareme1ForGrade(grade, refs.baremes1)
      const anneeLigne =
  ...
  function handleProduire() {
    ...
    if (bulletinsMois.length === 0) {
      window.alert(
        `Aucun enregistrement PaieMensuelleB1 pour ${moisLabel}.\n` +
          `Calculez d’abord la paie mensuelle Barème 1 dans Préparation de la paie.`,
      )
    }
  }
    ...
    <div className="bulletin-ancien-bareme-page">
      <header className="page-header bulletin-ancien-header no-print">
        <div>
          ...
          <h2>Bulletin avec ancien barème</h2>
          <p className="page-lead">
            Lecture du fichier PaieMensuelleB1 (Barème 1) et production des
      ...
      <section
        ...
        aria-labelledby="bulletin-ancien-list-title"
      >
        <div className="admin-list-head bulletin-ancien-list-head no-print">
          <div className="admin-list-title">
            ...
            <div>
              <h2 id="bulletin-ancien-list-title">
                Liste — Bulletins (ancien barème)
              </h2>
              <p>
                {produit
        ...
        <header className="bulletin-ancien-print-header print-only">
          <h1>Bulletins de paie — Ancien barème</h1>
          <p>{moisLabel}</p>
        </header>
        ...
        <div className="bulletin-ancien-table-area" ref={tableAreaRef}>
          <DataTable
            ...
            emptyMessage={
              produit
                ? `Aucun bulletin PaieMensuelleB1 pour ${moisLabel}.`
                : 'Cliquez sur « Produire les bulletins » pour lire PaieMensuelleB1.'
            }
            columns={[
              {
                key: 'actions',
